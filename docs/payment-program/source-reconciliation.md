...
|---|---|---|---|---|---|
| `SRC-PAY-01` | Final MVP Spec §30/§63; approved decisions §Payments; Payment UX Contract | Канонические документы говорят, что провайдер не выбран и включают ЕРИП; утверждённый target contract выбирает WEBPAY hosted same-tab checkout и откладывает ЕРИП | Product Owner / Payments | `A-04` | Feature implementation заблокирована до принятия A-04 |
| `SRC-PAY-02` | Final MVP Spec §31; legacy Payment schema/report; audit `SEC-08` | Каноническая модель допускает `provider_payload_json`, а целевой security boundary запрещает raw provider payload/secret persistence | Security / Payments | `A-04`, `A-05`, затем `B3-04` | Production `NO-GO` |
| `SRC-PAY-03` | README; approved decisions; E-POS report; WebPay smoke docs | README/approved decisions называют провайдера невыбранным, E-POS report описывает legacy scaffold, а WebPay docs — assumed sandbox adapter | Program governance / Payments | `A-05` | Ни один sandbox document не является merchant-approved protocol |
