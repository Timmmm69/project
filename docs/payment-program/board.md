...
Текущий gate: review A-05 вернул `CHANGES_REQUIRED` только по finding `A05-DOC-HYGIENE-01`; требуется отдельный correction pass. Остальные 11 dependency-ready карточек остаются открыты. Production остаётся `NO-GO`.
...
| [A-05](tasks/A-05.md) | A — Управление и документация | Создать launch-control и WEBPAY evidence documents | `CHANGES_REQUIRED` | `HIGH` | `A-03`, `A-04` |
