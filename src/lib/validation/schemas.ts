...
export const createTestBaseSchema = z.object({
  ...
  examMode: examModeSchema.default("generic"),
  subjectCode: z.string().trim().max(50).nullable().optional(),
  officialYear: z.number().int().min(2000).max(2100).nullable().optional(),
