...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const code = error instanceof Error ? error.message : "ATTEMPT_EXPIRE_FAILED";
    const messages: Record<string, string> = {
      ATTEMPT_NOT_FOUND: "Attempt not found",
      ATTEMPT_TIME_NOT_EXPIRED: "Attempt time has not expired yet"
    };
    const status = code === "ATTEMPT_NOT_FOUND" ? 404 : 409;
    return apiFailure({ code, message: messages[code] ?? "Attempt cannot be expired" }, status);
  }
}
